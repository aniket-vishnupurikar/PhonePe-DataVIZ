...
mydb = mysql.connector.connect(host="localhost", user="root", password="mysql", database="phonepe")
mycursor = mydb.cursor(buffered=True)

mycursor.execute('SELECT * FROM Data_Aggregated_Transaction_Summary_Table')
table_rows = mycursor.fetchall()
Data_Aggregated_Transaction_Summary_Table = pd.DataFrame(table_rows)
Data_Aggregated_Transaction_Summary_Table.columns = ['Data From', 'Data To', 'Quarter', 'State', 'Year']

mycursor.execute('SELECT * FROM Data_Aggregated_Transaction_Table')
table_rows = mycursor.fetchall()
Data_Aggregated_Transaction_df = pd.DataFrame(table_rows)
Data_Aggregated_Transaction_df.columns = ['Payment Mode', 'Quarter', 'State', 'Total Amount',
       'Total Transactions count', 'Year']

mycursor.execute('SELECT * FROM Data_Aggregated_User_Summary_Table')
table_rows = mycursor.fetchall()
Data_Aggregated_User_Summary_df = pd.DataFrame(table_rows)
Data_Aggregated_User_Summary_df.columns = ['AppOpenings', 'Quarter', 'Registered Users', 'State', 'Year']

mycursor.execute('SELECT * FROM Data_Aggregated_User_Table')
table_rows = mycursor.fetchall()
Data_Aggregated_User_df = pd.DataFrame(table_rows)
Data_Aggregated_User_df.columns = ['Brand Name', 'Percentage Share of Brand', 'Quarter',
       'Registered Users Count', 'State', 'Year']

mycursor.execute('SELECT * FROM Data_Map_Transaction_Table')
table_rows = mycursor.fetchall()
Data_Map_Transaction_df = pd.DataFrame(table_rows)
Data_Map_Transaction_df.columns = ['Place Name', 'Quarter', 'State', 'Total Amount',
       'Total Transactions count', 'Year']

mycursor.execute('SELECT * FROM Data_Map_User_Table')
table_rows = mycursor.fetchall()
Data_Map_User_Table = pd.DataFrame(table_rows)
Data_Map_User_Table.columns = ['App Openings', 'Place Name', 'Quarter', 'Registered Users Count',
       'State', 'Year']
...
with colT2:
    st.markdown("<h1 style='text-align: center; color: yellow;'>PhonePe Pulse Data Analysis</h1>",
...
# Plotting India Map and transactions
...
with colT2:
    st.info(
        """
        Map Guide:
        - The saturation level of the color of the circles represents the amount of total transactions instates
        - The Size of the Circles represents the total number iof transactions district wise
        - If you Hover over the data points the tooltips will show the detailed data like Total transactions, Total amount
        """
    )
...
with st.expander("See Bar graph for this data"):
    st.plotly_chart(fig, use_container_width=True)
    st.info(
        '**:red[No. of PhonePe Transactions by states]**')

# TRANSACTIONS ANALYSIS
st.write('# :red[TRANSACTIONS ANALYSIS]')
tab1, tab2, tab3, tab4 = st.tabs(["State Data", "District Data", "Year Wise Data", "Summarized"])
...
with tab1:
    ...
    with colT2:
        st.info(
            """
            Details of BarGraph:
            - The data shown depends on your selection
            - X Axis is date 
            - Y Axis is total transactions        
            """
        )

#FIGURE2 DISTRICTS ANALYSIS
with tab2:
...
with tab3:
    ...
    with colT2:
        st.info(
            """
            Details of BarGraph:
            - This data shown depends on selection made by the user
            - X Axis is all the states
            - Y Axis represents total transactions       
            """
        )
    ...
    with colT2:
        if l:
            st.info(
                """
                Details of BarGraph:
                - This data shown de[emds om user selection
                - X Axis represents the districts of selected state
...
with tab3:
    ...
    with colT2:
        st.info(
            ...
            - Percentage of registered users is represented with donut chart through Device Brand
            """
        )
...
with col1:
    ...
    st.markdown(rt[['State', 'Registered_Users']].style.hide_index().to_html(), unsafe_allow_html=True)
with col2:
    at = top_states_a[1:4]
    st.markdown("#### :orange[PhonePeApp Openings]")
    st.markdown(at[['State', 'AppOpenings']].style.hide_index().to_html(), unsafe_allow_html=True)
with col3:
